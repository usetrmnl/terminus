...
RSpec.describe Terminus::Aspects::Extensions::Importers::Remote::Transformer do
  ...
  describe "#call" do
    ...
    it "fails when serverless transform is detected" do
      archive[:transform] = "danger"
      allow(extractor).to receive(:call).and_return Success(archive)

      expect(transformer.call(1)).to be_failure("Serverless transforms are not supported yet.")
    end
