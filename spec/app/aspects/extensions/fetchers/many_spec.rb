...
RSpec.describe Terminus::Aspects::Extensions::Fetchers::Many do
  subject(:multi_fetcher) { described_class.new exchange_repository:, sole: }

  let(:sole) { instance_double Terminus::Aspects::Extensions::Fetchers::Sole }
  ...
  describe "#call" do
    ...
    it "answers success with single exchange" do
      allow(sole).to receive(:call).with(exchange).and_return(Success(exchange))
    ...
    it "answers success (empty hash) with single exchange fetch failure" do
      allow(sole).to receive(:call).with(exchange).and_return(Failure(exchange))
    ...
    it "answers success (partial hash) with multiple fetch successes and failures" do
      ...
      allow(sole).to receive(:call).with(exchange)
                                   .and_return(
                                     Success(exchange),
                                     Failure(exchange),
                                     Success(exchange)
