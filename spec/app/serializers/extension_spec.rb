...
RSpec.describe Terminus::Serializers::Extension, :db do
  ...
  describe "#to_h" do
    ...
    it "answers hash with model and device IDs" do
      extension_model = Factory[:extension_model, extension:]
      extension_device = Factory[:extension_device, extension:]
      ...
      expect(serializer.to_h).to match(
        ...
        created_at: match_rfc_3339,
        updated_at: match_rfc_3339,
        model_ids: [extension_model.model_id],
        device_ids: [extension_device.device_id]
      )
    end
  end
end
