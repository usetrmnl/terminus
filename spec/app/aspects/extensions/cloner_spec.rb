...
RSpec.describe Terminus::Aspects::Extensions::Cloner, :db do
  ...
  describe "#call" do
    ...
    it "clones extension with exchanges" do
      Factory[:extension_exchange, extension_id: original.id, uri: "https://test.io"]
      clone = cloner.call(original.id).value!
      uris = Terminus::Repositories::ExtensionExchange.new.where(extension_id: clone.id).map(&:uri)

      expect(uris).to contain_exactly("https://test.io")
    end
