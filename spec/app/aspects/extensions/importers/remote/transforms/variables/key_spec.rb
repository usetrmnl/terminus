...
RSpec.describe Terminus::Aspects::Extensions::Importers::Remote::Transforms::Variables::Key do
  ...
  describe "#call" do
    ...
    it "answers TRMNL device friendly ID as device label" do
      expect(transformer.call(+"trmnl.device.friendly_id")).to eq("extension.device.label")
    end

    it "answers TRMNL device percent charged as device battery percentage" do
      expect(transformer.call(+"trmnl.device.percent_charged")).to eq(
